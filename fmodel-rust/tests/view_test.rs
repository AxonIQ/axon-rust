...
fn test() {
    ...
    assert_eq!(new_state, OrderViewState {
        order_id: 1,
        customer_name: "John Doe".to_string(),
        items: vec!["Item 1".to_string(), "Item 2".to_string()],
        is_cancelled: false,
    ...
    assert_eq!(new_state, OrderViewState {
        order_id: 1,
        customer_name: "John Doe".to_string(),
        items: vec!["Item 11".to_string(), "Item 22".to_string(), "Item 33".to_string()],
        is_cancelled: false,
    });

    let order_canceled_event = OrderEvent::Cancelled(OrderCancelledEvent {
        order_id: 1
    });
